...
class FPN(nn.Module):
    def __init__(self, norm_layer, num_filters=256):
        """Creates an `FPN` instance for feature extraction. 创建用于特征提取的FPN网络
        Args:
        ...
        super().__init__()
        # 用的是inceptionv2网络做backbones
        self.inception = inceptionresnetv2(num_classes=1000, pretrained=None)

        # self.enc0 = self.inception.conv2d_1a
        self.enc0 = BasicConv2d(4, 32, kernel_size=3, stride=2)  # 32
    ...
    def unfreeze(self):
        for param in self.inception.parameters():  # 对权重的参数进行解冻
            param.requires_grad = True

    def forward(self, x):
        # 模型的前向推理，实际上就是调用子模型的__call__方法。
        ...
        map2 = self.td2(
            F.pad(lateral2, pad, "reflect") + nn.functional.interpolate(map3, scale_factor=2, mode="nearest"))
...
class FPNInception(nn.Module):
    ...
    def forward(self, x):
        ...
        smoothed = nn.functional.interpolate(smoothed, scale_factor=2, mode="nearest")  # 通过差值上取样
        smoothed = self.smooth2(smoothed + map0)  # 实际上这不是深度上的连接，而是残差连接
        smoothed = nn.functional.interpolate(smoothed, scale_factor=2, mode="nearest")  # 通过差值上取样

        final = self.final(smoothed)  # 最后的卷积层

        res = torch.tanh(final) + x[:, 0:3, :, :]  # 通过tanh层，然后此时把输入的x去c（深度维的前三个通道，对x处理然后加上就行了）
